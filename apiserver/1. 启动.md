...
# 4. 创建APIServer
创建APIServer的过程简单来说就是创建webservice, 给每个资源创建一个route, 并绑定handlers方法, handlers方法能够操作etcd. 然后将route添加到webservice中, 把webservice添加到container中. 这样当http请求到来时, 便可根据url来操作对应资源的etcd.

此外会通过装饰器方法, 给handlers包装上认证/授权/准入控制器, 使得操作etcd前, 先对请求进行认证/授权/准入控制.

创建APIServer的流程为:
1. 创建GenericAPIServer
2. 实例化APIGroup, 一个组对应着一个APIGroup, APIGroup包含着本组下所有资源, 以及资源对应的storage, storage能够以restful形式操作etcd
3. 通过InstallAPIGroup将gvr对应的url和storage注册到APIExtersionServerHandler中, 进而添加到go-restful的route里.
